...
def modal_salir():
    ...
    result = messagebox.askokcancel("Finalizar evalucion", "salir de la evaluacion perjudicara tu nota ¿estas seguro que deseas salir?")
    if result:
        global corriendo
        corriendo = False
        salir_programa()
...
def encriptacion(nombre_ususario, apellido_ususario, cedula_ususario, respuestas_examen):
    ...
    nombre_archivo = "{}_{}_{}.zip".format(nombre_ususario, apellido_ususario, cedula_ususario)

    # Crear un archivo temporal para almacenar las respuestas
    with open("temp.txt", "w") as archivo_temp:
        for pregunta, respuesta in zip(preguntas, respuestas_examen):
            archivo_temp.write("{}: {}\n".format(pregunta, respuesta))

    # Comprimir y encriptar el archivo temporal
    with zipfile.ZipFile(nombre_archivo, "w", zipfile.ZIP_DEFLATED) as archivo_zip:
        archivo_zip.write("temp.txt")

    # Eliminar el archivo temporal
    os.remove("temp.txt")

    # Mostrar un mensaje de éxito al usuario
    print("El archivo {} se ha creado correctamente.".format(nombre_archivo))
